...
##    # ruido de distribucion de objetos # ELIMINAR
...
#        self.db=None # ELIMINAR
        self.pool_modelos=dict() # {id:Model,...}; id="nombre_archivo" <- hashear!!!
#        self.ruido_perlin=PerlinNoise2(Objetos.ParamsRuido[0], Objetos.ParamsRuido[0], 256, Objetos.ParamsRuido[1]) # ELIMINAR
        # variables externas:
...
##    def update(self, forzar=False): # ELIMINAR
#        #
...
#                self._generar_parcela(idx_pos)
    ...
    def _generar_parcela(self, idx):
        ...
        log.info("_generar_parcela idx=%s pos=%s lod(sistema)=%i nombre=%s"%(str(idx), str(pos), lod,  nombre))
##        # datos de parcela # ELIMINAR
...
##    def _generar_datos_parcela(self, pos, idx_pos): # ELIMINAR
...
#                       self._chequear_espacio_disponible(d.posicion_rel_parcela, tipo_objeto[4], tipo_objeto[5], datos_locales):
...
#
##    def _obtener_vecino(self, datos_locales, posicion_rel_parcela, dx, dy): # ELIMINAR
#        if (posicion_rel_parcela[0]==0 and dx<0) or \
#           (posicion_rel_parcela[0]==(len(datos_locales[0])-1) and dx>0) or \
#           (posicion_rel_parcela[1]==0 and dy<0) or \
#           (posicion_rel_parcela[1]==(len(datos_locales[1])-1) and dy>0):
#               return None
#        return datos_locales[int(posicion_rel_parcela[0])+dx][int(posicion_rel_parcela[1])+dy]
#
##    def _chequear_espacio_disponible(self, _pos_parcela, radio_inferior, radio_superior, datos_locales): # ELIMINAR
...
#    #
##    def _terminar_db(self): # ELIMINAR
...
#
#
# DATOS LOCALES DE OBJETOS # ELIMINAR
#
#
#class DatosLocalesObjetos:
#    
#    def __init__(self, posicion_global, posicion_rel_parcela, ambiente, tipo_terreno):
#        self.posicion_global=posicion_global
#        self.posicion_rel_parcela=posicion_rel_parcela
...
#                %(str(self.posicion_global), str(self.posicion_rel_parcela), self.ambiente, self.tipo_terreno, self.factor_ruido, str(self.datos_objeto))

#
#
# GRUPO OBJETOS LOCACIONES # ELIMINAR
#
#
